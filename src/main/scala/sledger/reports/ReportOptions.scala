...
import sledger.data.Dates.{DateSpan, Exact, Interval, NoInterval, nulldate, nulldatespan, spanDefaultsFrom, spanEnd, spanStart, spanUnion, splitSpan}
...
object ReportOptions {
  ...
  case class Spec(options: Options, day: LocalDate)

  def defaultSpec: Spec = Spec(defaultsOptions,nulldate)
  
  def reportSpan(journal: Journal, spec: Spec): (DateSpan, List[DateSpan]) ={
    reporSpanHelper(bothdates = false, journal, spec)
  }
  
  def reporSpanHelper(bothdates: Boolean, journal: Journal, spec: Spec): (DateSpan, List[DateSpan]) = {
